...
public class PlayerInventory : MonoBehaviour
{
    ...
    [HideInInspector] public Deplacement deplacement;
    [HideInInspector] public Animation playerAnimations;
    [HideInInspector] public PlayerSkills playerSkills;
    ...
    void Start()
    {
        ...
        goldText = GameObject.Find("playerGoldTxt").GetComponent<Text>();

        deplacement = gameObject.GetComponent<Deplacement>();
    ...
    public void OnConsumeItem(Item item)
    {
        for (int i = 0; i < item.itemAttributes.Count; i++)
        {
            if (item.itemAttributes[i].attributeName == "Health" && currentHealth < maxHealth && !deplacement.isDead)
            {
            ...
            if (item.itemAttributes[i].attributeName == "Mana" && currentMana < maxMana && !deplacement.isDead)
            {
    ...
    void Update()
    {
        goldText.text = "Gold Coins : " + goldCoins;
        ...
        experienceBar.rectTransform.localScale = new Vector3(percentageXP, 1, 1);
        ...
        if (percentageHP < 0)
            percentageHP = 0;
        hpImage.rectTransform.localScale = new Vector3(percentageHP, 1, 1);

        float percentageMana = ((currentMana * 100) / maxMana) / 100;
        manaImage.rectTransform.localScale = new Vector3(percentageMana, 1, 1);

        if (Input.GetKeyDown(inputManagerDatabase.CharacterSystemKeyCode) && !chestIsOpen)
        {
        ...
        if (Input.GetKeyDown(inputManagerDatabase.InventoryKeyCode) && !chestIsOpen)
        {
    ...
    public IEnumerator WaitForRespawn()
    {
        ...
        currentHealth = maxHealth;
        deplacement.isDead = false;
        if (currentDamage != 0 && goldCoins != 0 && currentXP != 0)
        {
            goldCoins = Mathf.RoundToInt(goldCoins / 1 / currentDamage);
        }
    }
}
